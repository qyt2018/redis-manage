...
export default {
  ...
  methods: {
    ...
    save: function () {
      ...
      let oldVal = ''
      if (this.index !== null) {
        oldVal = this.value[this.index]
      }
      $.ajax({
        type: 'post',
        dataType: 'json',
        data: {action: 'save', type: 'set', ikey: this.ikey, index: this.index, val: this.val, old_val: oldVal},
    ...
    delItem: function () {
      $.ajax({
        type: 'post',
        dataType: 'json',
        data: {action: 'del', type: 'set', ikey: this.ikey, index: this.index, val: this.val},
        url: 'http://' + document.location.host
      }).done((resp) => {
        if (resp.err === '0') {
          if (!this.val) {
            this.$emit('listenSet', this.ikey)
          } else {
